...
export class ArticleController {
    ...
    async createArticle(
        ...
        this.logger.verbose(`User ${logginedUser.username} creating a new Article. Data: ${JSON.stringify(createArticleRequestDto)}`);
        const newArticle = await this.articleService.createArticle(createArticleRequestDto, logginedUser, file);
        const articleWithAttachmentAndUserResponseDto = new ArticleWithAttachmentAndUserResponseDto(newArticle);
        this.logger.verbose(`Article created successfully: ${JSON.stringify(articleWithAttachmentAndUserResponseDto)}`);
        return new ApiResponse(true, 201, 'Article created successfully', articleWithAttachmentAndUserResponseDto);
    }
    ...
    async updateArticleById(
        ...
        const updatedArticle = await this.articleService.updateArticleById(id, updateArticleRequestDto, logginedUser, file);
        const articleWithAttachmentAndUserResponseDto = new ArticleWithAttachmentAndUserResponseDto(updatedArticle);
        this.logger.verbose(`Article updated successfully: ${JSON.stringify(articleWithAttachmentAndUserResponseDto)}`);
        return new ApiResponse(true, 200, 'Article updated successfully', articleWithAttachmentAndUserResponseDto);
    }
}